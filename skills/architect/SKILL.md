...
Use `todo` with `action: "set"` and one top-level object seed per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use `job.role: "architect-runners"` with successive `panelIndex` values for architecture runners and `arena-runners` for structurally distinct implementation candidates.

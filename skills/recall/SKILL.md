...
Use `pstack_sessions` for transcript and worker evidence. Start with `action: "list"` for this workspace and `action: "jobs"`; read only candidate tails by ID. The tool enforces workspace scope and labels returned content as untrusted.
...
3. Mine the bounded workspace history. Call `pstack_sessions` with `action: "list"`, ordered by modification time, excluding the current session. Read likely candidates with `action: "tail"`; inspect matching worker evidence through `jobs` and `job-tail`. Skip obvious eval and test noise. For each relevant chat capture: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), citing the session ID. Keep raw historical content out of the final context after extracting those facts.

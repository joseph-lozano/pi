...
2. Pick a managed background wake mechanism. An event to watch gets a shell `job` with `wake: "always"`; PR checks use `gh pr checks <number> --watch --interval 30`. When no event source exists, use a one-shot background sleep/check command sized to when the result is worth rechecking. On completion, act and explicitly rearm only if the exit predicate is still false.

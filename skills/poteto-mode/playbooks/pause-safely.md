...
1. Stop at a safe boundary. Finish the current atomic step or back out of it. Never stop mid-edit in a known-broken state. Start nothing new. Inspect managed jobs with `job status` and stop only jobs owned by this session that cannot safely continue in the background.
...
4. Write the resume note off-context. Capture intent, what you were doing, progress and what's verified, current state, next steps, key files, and gotchas. For the compaction trigger, rely on the persisted todo for playbook position and write durable details that do not fit there to `/tmp/<slug>-resume.md`. Pi preserves session custom entries across compaction, but a cold-start agent still needs concise off-context state. If a show-me-your-work trail exists, point at it instead of duplicating it.

...
7. **High-level comparison with Pi’s `context` hook (important implementation constraint).** Pi fires `context` before every LLM call and gives handlers a deep copy of `event.messages`, safe to filter or rewrite by returning `{messages}`. That is sufficient to preserve assistant tool calls while transiently replacing paired results or large spans for the model. It is explicitly non-destructive, however: `ctx.sessionManager` is read-only, so the hook itself cannot rewrite persisted branch entries, create omp-style `artifact://` objects, synchronize stored token accounting, or perform omp’s provider/advisor lifecycle resets. A faithful Pi feature therefore needs state/persistence machinery beyond a `context` handler; a hook-only version is an approximation whose substitutions are recomputed for each call and whose original session remains unchanged. [Pi extension docs: `context` and read-only `sessionManager`](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/docs/extensions.md) · [Pi session/message format](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/docs/session-format.md)

/**
 * Mask secrets in `read` tool results using rules from ~/.pi/agent/cloak.json.
 *
 * Only rewrites text from the built-in `read` tool (not bash/cat). Reload
 * config on session_start; `/cloak-status` shows path + pattern count.
 *
 * Ported from Dillon Mulroy's pi-cloak (dmmulroy/.dotfiles).
 */
